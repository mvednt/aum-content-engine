...
/**
 * GEMINI_MODEL may hold several names, comma separated, best first.
 * A named model can be retired for new keys (404) or simply overloaded (503),
 * so the engine walks the list rather than failing on the first name.
 */
...
/**
 * Gemini. `parts` is an array of {text} and/or {inline_data} objects,
 * so the same function handles a text note and a voice note.
 */
export async function gemini(parts, { model, temperature = 0.7, system = VOICE, envName = 'GEMINI_MODEL' } = {}) {
  ...
  for (const name of names) {
    try {
      ...
      // A 400 means the request itself is wrong; another model will not fix it.
...
/** Claude. Used for drafting from B1 onwards: it holds a voice across a full post. */
...
/**
 * The drafting model is one switch. DRAFT_MODEL=claude moves drafting to Claude
 * and leaves scoring and keyword extraction on Gemini Flash, which is the split
 * the course stack argues for: cheap mechanical work stays cheap.
 */
 ...
 * so the same function handles a text note and a voice note.
 */
export async function gemini(parts, { model, temperature = 0.7, system = VOICE } = {}) {
...
export async function draftModel(prompt) {
  ...
  return gemini([{ text: prompt }], {
    model: process.env.GEMINI_DRAFT_MODEL || process.env.GEMINI_MODEL || 'gemini-2.5-flash',
    temperature: 0.85,
  });
}
